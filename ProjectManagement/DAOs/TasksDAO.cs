    ...
    internal class TasksDAO : DBConnection
    {
        ...
        public List<Tasks> SelectListByTeam(string idTeam)
        {
            string command = string.Format("SELECT * FROM {0} WHERE idteam = '{1}' ORDER BY created DESC",
                                            DBTableNames.DBTask, idTeam);
            return SelectList(command);
        }
        public Tasks SelectOnly(string idTask)
        {
            DataTable dt = Select(string.Format("SELECT * FROM {0} WHERE idtask = '{1}'", DBTableNames.DBTask, idTask));
        ...
        {
            DataTable dt = Select(string.Format("SELECT * FROM {0} WHERE idcomment = '{1}'", DBTableNames.DBComment, idComment));
        ...
        {
            DataTable dt = Select(string.Format("SELECT * FROM {0} WHERE idevaluation = '{1}'", DBTableNames.DBEvaluation, idEvaluation));
        ...
        {
            SQLExecuteByCommand(string.Format("UPDATE " + DBTableNames.DBTask + " SET isfavorite = {0} WHERE idtask = '{1}'",
        ...
        public List<Tasks> SearchTaskTitle(string idTeam, string title)
        {
            string command = string.Format("SELECT * FROM {0} WHERE idteam = '{1}' and title LIKE '{2}%' ORDER BY created DESC",
                                DBTableNames.DBTask, idTeam, title);
            return SelectList(command);
