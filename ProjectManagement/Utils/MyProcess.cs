        ...
        public string GenIDClassify(EClassify eClassify)
        {
            ...
            switch (eClassify)
            {
                case EClassify.Lecture:
                    cntAccount = GetLastestID(string.Format("SELECT max(idaccount) as MaxID FROM {0} WHERE role = '{1}'", DBTableNames.DBPeople, EClassify.Lecture.ToString()));
                    break;
                case EClassify.Student:
                    cntAccount = GetLastestID(string.Format("SELECT max(idaccount) as MaxID FROM {0} WHERE role = '{1}'", DBTableNames.DBPeople, EClassify.Student.ToString()));
                    break;
                case EClassify.Team:
                    cntAccount = GetLastestID(string.Format("SELECT max(idteam) as MaxID FROM {0}", DBTableNames.DBMember));
                    break;
                case EClassify.Thesis:
                    cntAccount = GetLastestID(string.Format("SELECT max(idthesis) as MaxID FROM {0}", DBTableNames.DBThesis));
                    break;
                case EClassify.Task:
                    cntAccount = GetLastestID(string.Format("SELECT max(idtask) as MaxID FROM {0}", DBTableNames.DBTask));
                    break;
                case EClassify.Comment:
                    cntAccount = GetLastestID(string.Format("SELECT max(idcomment) as MaxID FROM {0}", DBTableNames.DBComment));
                    break;
                case EClassify.Evaluation:
                    cntAccount = GetLastestID(string.Format("SELECT max(idevaluation) as MaxID FROM {0}", DBTableNames.DBEvaluation));
                    break;
                case EClassify.Notification:
                    cntAccount = GetLastestID(string.Format("SELECT max(idnotification) as MaxID FROM {0}", DBTableNames.DBNotification));
                    break;
                case EClassify.Meeting:
                    cntAccount = GetLastestID(string.Format("SELECT max(idmeeting) as MaxID FROM {0}", DBTableNames.DBMeeting));
                    break;
            }
            cntAccount++;
